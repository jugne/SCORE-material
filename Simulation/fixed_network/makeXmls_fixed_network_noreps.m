% makes simulation xmls for the coalescent with reassortment in structured
% populations
clear
...
system('rm -r inference_noReps');
system('mkdir inference_noReps');
...
h = fopen('rates_noReps.csv', 'w');fprintf(h, 'run,Ne_1,Ne_2,reassortment_1,reassortment_2,migration_1, migration_2 \n');
...
%             fprintf(g, strrep(line, 'insert_Ne', num2str(Ne_1)+" "+num2str(Ne_2)));
...
% 			g = fopen(sprintf('inference/inf_%d_rep%d.xml',  i, r), 'w');
            g = fopen(sprintf('inference_noReps/inf_%d.xml',  i), 'w');
            ...
            while ~feof(f_inf)
                ...
                elseif contains(line, 'initial_Ne')
                    fprintf(g, strrep(line, 'initial_Ne', num2str(exprnd(1))+" "+num2str(exprnd(1))));
                elseif contains(line, 'simulated_coalescence')
                    fprintf(g, strrep(line, 'simulated_coalescence', num2str(1/Ne_1)+" "+num2str(1/Ne_2)));
                elseif contains(line, 'initial_reassortment')
                    fprintf(g, strrep(line, 'initial_reassortment', num2str(exprnd(1))+" "+num2str(exprnd(1))));
                elseif contains(line, 'simulated_reassortment')
                    fprintf(g, strrep(line, 'simulated_reassortment', num2str(reassortment_1)+" "+num2str(reassortment_2)));
                elseif contains(line, 'initial_migration')
                    fprintf(g, strrep(line, 'initial_migration',num2str(exprnd(1))+" "+num2str(exprnd(1))));
                elseif contains(line, 'simulated_migration')
                    fprintf(g, strrep(line, 'simulated_migration',  num2str(migration_1)+" "+num2str(migration_2)));
%                elseif contains(line, 'insert_sim_file_name')
%                    line = strrep(line, 'insert_sim_file_name', sprintf('../simulation/sim_%d', i) );
