...
true_rates_file <- "rates_noReps.csv"
...
# for (g in seq(1,length(gen_information))){
  ...
  log <- list.files(path="./inference_noReps/", pattern="inf_.*.log", full.names = TRUE)
  ...
  for (i in seq(1,length(log))){
    ...
    if (post_ess>100){
      ...
      new.rea_1 = data.frame(true=used.rates$reassortment_1, estimated=mean(t$reassortmentRate1), 
                      upper=quantile(t$reassortmentRate1,0.975), lower=quantile(t$reassortmentRate1,0.025) )
      new.rea_2 = data.frame(true=used.rates$reassortment_2, estimated=mean(t$reassortmentRate2), 
                             upper=quantile(t$reassortmentRate2,0.975), lower=quantile(t$reassortmentRate2,0.025) )
      new.mig_1 = data.frame(true=used.rates$migration_1, estimated=mean(t$migrationRate1), 
                             upper=quantile(t$migrationRate1,0.975), lower=quantile(t$migrationRate1,0.025) )
      new.mig_2 = data.frame(true=used.rates$migration_2, estimated=mean(t$migrationRate2), 
                             upper=quantile(t$migrationRate2,0.975), lower=quantile(t$migrationRate2,0.025) )
      new.Ne_1 = data.frame(true=used.rates$Ne_1, estimated=mean(t$popSize1), 
                      upper=quantile(t$popSize1,0.975), lower=quantile(t$popSize1,0.025))
      new.Ne_2 = data.frame(true=used.rates$Ne_2, estimated=mean(t$popSize2), 
                            upper=quantile(t$popSize2,0.975), lower=quantile(t$popSize2,0.025))
      
      
      hpd.rea_1 = HPDinterval(as.mcmc(t$reassortmentRate1))
      hpd.rea_2 = HPDinterval(as.mcmc(t$reassortmentRate2))
      hpd.mig_1 = HPDinterval(as.mcmc(t$migrationRate1))
      hpd.mig_2 = HPDinterval(as.mcmc(t$migrationRate2))
      hpd.Ne_1 = HPDinterval(as.mcmc(t$popSize1))
      hpd.Ne_2 = HPDinterval(as.mcmc(t$popSize2))
  ...
  p.rea_2 <- ggplot(rea_2)+
    ...
    xlab("True parameter value") +
    ylab("Mean of posterior (approx.)") +
    ylim(y.min_rea, y.max_rea) +
    xlim(x.min_rea, x.max_rea) +
    theme(text = element_text(size=20))+
    scale_y_log10(limits=c(y.min_rea, y.max_rea)) +
    scale_x_log10(limits=c(x.min_rea, x.max_rea)) 
  ...
  p.mig_2 <- ggplot(mig_2)+
    ...
    ylab("Mean of posterior (approx.)") +
    theme(text = element_text(size=20))+
    ylim(y.min_mig, y.max_mig) +
    xlim(x.min_mig, x.max_mig) +
    scale_y_log10(limits=c(y.min_mig, y.max_mig)) +
    scale_x_log10(limits=c(x.min_mig, x.max_mig)) 
  ...
  ggsave(plot=p.Ne_1,paste("figures_fixed_network/type_1_Ne_sim_", ".pdf", sep=""),width=6, height=5)
  ggsave(plot=p.rea_1,paste("figures_fixed_network/type_1_rho_sim_", ".pdf", sep=""),width=6, height=5)
  ggsave(plot=p.mig_1,paste("figures_fixed_network/type_1_mig_sim_", ".pdf", sep=""),width=6, height=5)
  ggsave(plot=p.Ne_2,paste("figures_fixed_network/type_2_Ne_sim_", ".pdf", sep=""),width=6, height=5)
  ggsave(plot=p.rea_2,paste("figures_fixed_network/type_2_rho_sim_", ".pdf", sep=""),width=6, height=5)
  ggsave(plot=p.mig_2,paste("figures_fixed_network/type_2_mig_sim_", ".pdf", sep=""),width=6, height=5)
  ...
  write.csv(hpd.test, file = "HPD_test_10000_noReps_2.csv")
